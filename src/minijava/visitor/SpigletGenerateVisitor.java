...
*/
public class SpigletGenerateVisitor extends GJDepthFirst<String,TypeBasic> {
	...
	public SpigletGenerateVisitor(SymbolTableVisitor stv){
		symbolTable=stv;
		output="";
	}
...
public String visit(MainClass n, TypeBasic argu) {
   ...
   currentClass=symbolTable.classes.get(name);
...
public String visit(ArrayAssignmentStatement n, TypeBasic argu) {
	...
	String position=regName();
	String bytes=regName();
	String offset=regName();	
	String len=regName();
	String compare=regName();
	...
	printx("MOVE "+compare+" LT "+index+" "+len);
	printx("CJUMP "+compare+" "+outofbound);
	
	printx("MOVE "+position+" PLUS "+index+" 1");
	printx("MOVE "+offset+" TIMES "+position+" 4");
...
public String visit(CompareExpression n, TypeBasic argu) {
	...
	String exp2=n.f2.accept(this,argu);
	String ansReg=regName();
	printx("MOVE "+ansReg+" LT "+exp1+" "+exp2);
...
public String visit(PlusExpression n, TypeBasic argu) {
	String ansReg=regName();
	printx("MOVE "+ansReg+" PLUS "+n.f0.accept(this,argu)+" "+n.f2.accept(this,argu));
	return ansReg;
}
...
public String visit(MinusExpression n, TypeBasic argu) {
	String ansReg=regName();
	printx("MOVE "+ansReg+" MINUS "+n.f0.accept(this,argu)+" "+n.f2.accept(this,argu));
	return ansReg;
}
...
public String visit(TimesExpression n, TypeBasic argu) {
	String ansReg=regName();
	printx("MOVE "+ansReg+" TIMES "+n.f0.accept(this,argu)+" "+n.f2.accept(this,argu));
	return ansReg;
}
...
public String visit(MessageSend n, TypeBasic argu) {
	...
	if (n.f4.present()){
		...
		int paramSize=list.f1.nodes.size();
		if (paramSize<19){
			...
			for (Node node:list.f1.nodes){
				...
				if (paramCount<18){
					...
					params+=" "+pointer;
					printx("MOVE "+pointer+" HALLOCATE 4");
					tmpStore=node.accept(this,argu);
					printx("HSTORE "+pointer+" "+((paramCount-18)*4)+" "+tmpStore);
				}
				else{
					tmpStore=node.accept(this,argu);
					printx("HSTORE "+pointer+" "+((paramCount-18)*4)+" "+tmpStore);
...
public String visit(IntegerLiteral n, TypeBasic argu) {
	String ansReg=regName();
	printx("MOVE "+ansReg+" "+n.f0.toString());
	return ansReg;
}
...
public String visit(Identifier n, TypeBasic argu) {
	...
	if (tv!=null){
		aClass=tv.type;
		int index=currentMethod.getIndex(id);
		if (index<19 || currentMethod.pars.size()<20){
			String ansReg="TEMP "+index;
			return ansReg;
		}
		else {
			String ansReg=regName();
			printx("HLOAD "+ansReg+" TEMP 19 "+(index-19)*4);
			return ansReg;
		}
	}
...
public String visit(ArrayAllocationExpression n, TypeBasic argu) {
	...
	printx("MOVE "+len+" PLUS "+exp+" 1");
	//printx("NOOP");
	printx("MOVE "+allocate+" TIMES "+len+" 4");	
	printx("MOVE "+ansReg+" HALLOCATE "+allocate);
	printx("HSTORE "+ansReg+" 0 "+exp);
	
	//初始化数组所有元素为0
	String index=regName();
	String pointer=regName();
	String compare=regName();
	String zeros=regName();
	String initial=label();
	String endInit=label();
	//printx("MOVE "+pointer+" "+ansReg);
	printx("MOVE "+index+" 0");
	printx(initial);
	printx("MOVE "+index+" PLUS "+index+" 4");
	printx("MOVE "+compare+" LT "+index+" "+allocate);
	printx("CJUMP "+compare+" "+endInit);
	//printx("MOVE "+index+" PLUS "+index+" 4");
	printx("MOVE "+pointer+" PLUS "+ansReg+" "+index);
	printx("MOVE "+zeros+" 0");
	printx("HSTORE "+pointer+" 0 "+zeros);
	printx("JUMP "+initial);
	printx(endInit);
	printx("NOOP");
...
public String visit(AllocationExpression n, TypeBasic argu) {
	...
	int i=0;
	String funcname=regName();
	String funcs=regName();
	printx("MOVE "+funcs+" HALLOCATE "+(numMethod)*4);
	while(tc!=null){
		for (TypeMethod tm:tc.methods.values()){
			printx("MOVE "+funcname+" "+id+"_"+tm.name);
			printx("HSTORE "+funcs+" "+(i*4)+" "+funcname);
			i++;
		}
		tc=tc.parent;
	}
